...
Json::Value ABI::generate(ContractDefinition const& _contractDef)
{
	...
	{
		FunctionType constrType(*_contractDef.constructor(), FunctionType::Kind::External);
	...
	for (auto const* fallbackOrReceive: {_contractDef.fallbackFunction(), _contractDef.receiveFunction()})
		...
		{
			auto const* externalFunctionType = FunctionType(*fallbackOrReceive, FunctionType::Kind::External).interfaceFunctionType();
			solAssert(!!externalFunctionType, "");
			Json::Value method;
